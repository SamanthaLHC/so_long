...
/*   Updated: 2022/03/19 22:40:00 by sam              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void load_img(t_setup *setup, t_img *img)
{
	...
	img->img_char_face1 = mlx_xpm_file_to_image(setup->mlx, "./face.xpm", &x, &y);
    img->img_coll = mlx_xpm_file_to_image(setup->mlx, "./coll.xpm", &x, &y);
    img->img_exit = mlx_xpm_file_to_image(setup->mlx, "./exit.xpm", &x, &y);
...
void put_img_in_win(t_setup *setup, t_img *img)
{
    ...
    while (setup->save_in_tab[i])
    {
        ...
        if (setup->save_in_tab[i] == '1')
            ...
            mlx_put_image_to_window(setup->mlx, setup->win, img->img_coll, 
                ITOX(i, setup->line_size) * 48, ITOY(i, setup->line_size) * 48);  
        else if (setup->save_in_tab[i] == 'E')
            mlx_put_image_to_window(setup->mlx, setup->win, img->img_exit, 
                ITOX(i, setup->line_size) * 48, ITOY(i, setup->line_size) * 48);       
        i++;
    }
}